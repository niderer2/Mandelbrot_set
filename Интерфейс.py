...
def generate_mandelbrot():
    ...
    params = [
        ...
        ' '.join(fifa(vare_var[4])), # Модуль радиан, радианы это z / c
        ' '.join(fifa(vare_var[5])), # Отсудвие вращения при возведении на мнимое число
        ' '.join(fifa(vare_var[6])), # Вкл. общий вид при мнимой части в множителя точки
        ' '.join(fifa(vare_var[7])), # Умножение по отдельности
        ' '.join(fifa(vare_var[8])), # Брать цвет логарифмически, зависимость от радиуса, зависимость от z
        remove_n(variables[13].get()), #степень для логарифмического присваивания
    ]
    try:
...
params_info = [
    ...
    ('Степень для ологарифмического присваивания цвета (границы множества, z)', ('1.0 1.0')),
]
